...
# Start document - 2024.05

- ubuntu 22.04
- openvino version: 2024.1
- python version: 3.10
- level-zero updated to the latest version：https://github.com/intel/compute-runtime/releases
...
Doc：[Data Prepare](./DataPrepare.md)
```
...
##### 1. Prepare the environment (1.1-1.6)
##### 2. Modify the dataset path in the quantization script

```python
DATA_PATH=""
```

##### 3. Execute quantization script
...
##### 4. Save after quantification

The quantized model will be saved in the `tools` directory and named: `quantized_pfe.xml`
...
python infer.py
```
The configuration information can be modified in CONFIG in infer.py.
